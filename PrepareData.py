...
class STFTModule:
    ...
    def fwd(self, x):
        ...
        for i in range(frame_count):
            ...
            windowed_x = np.concatenate((self.window * x[start:start+self.sample_per_frame*2],np.zeros(self.fft_size - self.sample_per_frame * 2)))
    ...
    for target_file in args.target_file:
        if os.path.exists(target_file):
            ...
            if ('WAV'.find(tarfilesuffix.upper())) == 0:
                ...
                generate_feature_file(target_file, output_filepath, args.channel_num, args.sample_per_frame)
    end = datetime.now()
